...
├── app/                  # All source code (8 modules, no __init__.py — namespace pkg)
│   ├── main.py           # 3 webhook/admin routes + 3 AI admin routes + AI wiring
│   ├── config.py         # Hand-rolled env loading (NOT pydantic-settings)
│   ├── feishu_client.py  # tenant_access_token cache + bitable record PUT/POST + list_fields
│   ├── target_registry.py # dual-mode target routing + hot reload
...
│   ├── ai_profile.py     # TOML profile parser + hot-reloading registry + whitelist snapshot
│   ├── pipeline.py       # AI orchestration: extract → encode → writeback → create (never raises)
│   └── static/           # admin.html — AI config page served at /admin/ai
...
├── test/unit/            # pytest suite (100 tests); conftest pins env BEFORE app import
...
| Task | Location | Notes |
|------|----------|-------|
| Add/modify an API route | `app/main.py` | 6 routes: `/health`, `/v1/webhook/ocr`, `/admin/config/reload`, `/admin/ai`, `/admin/ai/test`, `/admin/ai/profile` |
| Change config/env vars | `app/config.py` | Hand-rolled env loading (NOT pydantic-settings) |
| Feishu API calls / token cache | `app/feishu_client.py` | tenant_access_token cached in-process, refresh skew 300s |
| Target routing / hot reload | `app/target_registry.py` | dual-mode: dynamic TOML vs legacy env |
...
| AI profile TOML + hot-reload registry | `app/ai_profile.py` | parse_profile + AiProfileRegistry (async, mtime reload) |
...
- Tests live under `test/unit/` (100 tests, pytest); conftest pins env BEFORE app import and clears the `get_settings` cache per test.
...
# Tests
python -m pytest                         # 100 tests under test/unit/
