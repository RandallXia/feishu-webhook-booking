...
**注意**：上述分工仅适用于 `AI_ENABLED=false`（默认）的情况。当 `AI_ENABLED=true` 时，`精简原始数据` 与 `账单明细` 由本服务的自托管 AI 管线接管，无需在飞书侧创建对应自动化；只有 `AI_ENABLED=false` 时才需要飞书侧自动化。详见 [AI 提取管线](ai-pipeline.md)。
